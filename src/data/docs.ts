export type DocMeta = {
  title: string;
  slug: string;
  order: number;
};

export const docs: DocMeta[] = [
  {
    title: "现代 Web 前端开发指南：2026",
    slug: "01-web-2026",
    order: 1,
  },
  {
    title: "0. 引言",
    slug: "02-0",
    order: 2,
  },
  {
    title: "I. 前端开发简介",
    slug: "03-i",
    order: 3,
  },
  {
    title:
      "II. 基础 Web 技术：核心支柱 / II.1 关于互联网 (Internet) 的基础知识",
    slug: "04-ii-web-ii-1-internet",
    order: 4,
  },
  {
    title: "II. 基础 Web 技术：核心支柱 / II.2 HTML：构建网页结构",
    slug: "05-ii-web-ii-2-html",
    order: 5,
  },
  {
    title: "II. 基础 Web 技术：核心支柱 / II.3 CSS：样式和布局",
    slug: "06-ii-web-ii-3-css",
    order: 6,
  },
  {
    title: "II. 基础 Web 技术：核心支柱 / II.4 现代 CSS 架构与策略",
    slug: "07-ii-web-ii-4-css",
    order: 7,
  },
  {
    title: "II. 基础 Web 技术：核心支柱 / II.5 JavaScript：赋予交互性生命",
    slug: "08-ii-web-ii-5-javascript",
    order: 8,
  },
  {
    title:
      "II. 基础 Web 技术：核心支柱 / II.6 TypeScript：为 JavaScript 注入类型安全与工程化能力",
    slug: "09-ii-web-ii-6-typescript-javascript",
    order: 9,
  },
  {
    title:
      "III. 基本开发环境和工具 / III.1 版本控制系统：Git 和 GitHub 协作开发",
    slug: "10-iii-iii-1-git-github",
    order: 10,
  },
  {
    title: "III. 基本开发环境和工具 / III.2 Git 工作流与团队协作",
    slug: "11-iii-iii-2-git",
    order: 11,
  },
  {
    title: "III. 基本开发环境和工具 / III.3 高级工程化工作流",
    slug: "12-iii-iii-3",
    order: 12,
  },
  {
    title:
      "III. 基本开发环境和工具 / III.4 包管理器：npm、Yarn 和 pnpm 依赖管理",
    slug: "13-iii-iii-4-npmyarn-pnpm",
    order: 13,
  },
  {
    title: "III. 基本开发环境和工具 / III.5 构建工具和打包器：优化生产代码",
    slug: "14-iii-iii-5",
    order: 14,
  },
  {
    title:
      "III. 基本开发环境和工具 / III.6 代码质量和一致性：Linter (ESLint) 和 Formatter (Prettier)",
    slug: "15-iii-iii-6-linter-eslint-formatter-prettier",
    order: 15,
  },
  {
    title: "III. 基本开发环境和工具 / III.7 集成开发环境 (IDE) 和编辑器",
    slug: "16-iii-iii-7-ide",
    order: 16,
  },
  {
    title: "III. 基本开发环境和工具 / III.8 新兴开发环境与 AI 驱动的工具",
    slug: "17-iii-iii-8-ai",
    order: 17,
  },
  {
    title: "III. 基本开发环境和工具 / III.9 浏览器开发者工具：调试和检查",
    slug: "18-iii-iii-9",
    order: 18,
  },
  {
    title:
      "III. 基本开发环境和工具 / III.10 从设计到代码的桥梁：前端开发者的设计协作实践",
    slug: "19-iii-iii-10",
    order: 19,
  },
  {
    title:
      "IV. 前端框架和库：构建现代 UI / IV.1 UI 框架概述：React、Vue、Angular、Svelte、SolidJS、Lit",
    slug: "20-iv-ui-iv-1-ui-reactvueangularsveltesolidjslit",
    order: 20,
  },
  {
    title:
      "IV. 前端框架和库：构建现代 UI / IV.2 Web Components：原生、跨框架的组件化未来",
    slug: "21-iv-ui-iv-2-web-components",
    order: 21,
  },
  {
    title: "IV. 前端框架和库：构建现代 UI / IV.3 UI 组件库：加速开发的利器",
    slug: "22-iv-ui-iv-3-ui",
    order: 22,
  },
  {
    title: "IV. 前端框架和库：构建现代 UI / IV.4 元框架：增强开发体验",
    slug: "23-iv-ui-iv-4",
    order: 23,
  },
  {
    title: "IV. 前端框架和库：构建现代 UI / IV.5 htmx：超媒体驱动开发",
    slug: "24-iv-ui-iv-5-htmx",
    order: 24,
  },
  {
    title:
      "IV. 前端框架和库：构建现代 UI / IV.6 状态管理解决方案：集中化应用程序数据",
    slug: "25-iv-ui-iv-6",
    order: 25,
  },
  {
    title:
      "IV. 前端框架和库：构建现代 UI / IV.7 数据获取和缓存库：SWR、TanStack Query、Axios、Fetch API",
    slug: "26-iv-ui-iv-7-swrtanstack-queryaxiosfetch-api",
    order: 26,
  },
  {
    title: "IV. 前端框架和库：构建现代 UI / IV.8 理解服务器状态与客户端状态",
    slug: "27-iv-ui-iv-8",
    order: 27,
  },
  {
    title: "IV. 前端框架和库：构建现代 UI / IV.9 现代后端集成模式",
    slug: "28-iv-ui-iv-9",
    order: 28,
  },
  {
    title: "V. 高级主题和专业开发最佳实践 / V.1 性能优化：提供快速用户体验",
    slug: "29-v-v-1",
    order: 29,
  },
  {
    title:
      "V. 高级主题和专业开发最佳实践 / V.2 线上性能监控与分析 (Performance Monitoring)",
    slug: "30-v-v-2-performance-monitoring",
    order: 30,
  },
  {
    title:
      "V. 高级主题和专业开发最佳实践 / V.3 Web 可访问性 (A11y)：为所有用户提供包容性设计",
    slug: "31-v-v-3-web-a11y",
    order: 31,
  },
  {
    title:
      "V. 高级主题和专业开发最佳实践 / V.4 搜索引擎优化 (SEO)：确保可发现性",
    slug: "32-v-v-4-seo",
    order: 32,
  },
  {
    title: "V. 高级主题和专业开发最佳实践 / V.5 安全基础：保护前端应用程序",
    slug: "33-v-v-5",
    order: 33,
  },
  {
    title: "V. 高级主题和专业开发最佳实践 / V.6 安全基础：用户认证与授权",
    slug: "34-v-v-6",
    order: 34,
  },
  {
    title: "V. 高级主题和专业开发最佳实践 / V.7 组件驱动开发与 Storybook",
    slug: "35-v-v-7-storybook",
    order: 35,
  },
  {
    title: "V. 高级主题和专业开发最佳实践 / V.8 测试策略：确保代码质量和可靠性",
    slug: "36-v-v-8",
    order: 36,
  },
  {
    title:
      "V. 高级主题和专业开发最佳实践 / V.9 国际化 (internationalization, i18n) 和本地化 (localization, l10n)：全球化应用程序",
    slug: "37-v-v-9-internationalization-i18n-localization-l10n",
    order: 37,
  },
  {
    title:
      "V. 高级主题和专业开发最佳实践 / V.10 错误处理和防御性编程：构建健壮的应用程序",
    slug: "38-v-v-10",
    order: 38,
  },
  {
    title: "V. 高级主题和专业开发最佳实践 / V.11 UX 工程原则：增强用户体验",
    slug: "39-v-v-11-ux",
    order: 39,
  },
  {
    title:
      "V. 高级主题和专业开发最佳实践 / V.12 前端架构模式：扩展复杂应用程序",
    slug: "40-v-v-12",
    order: 40,
  },
  {
    title: "V. 高级主题和专业开发最佳实践 / V.13 移动端 Web 开发实践",
    slug: "41-v-v-13-web",
    order: 41,
  },
  {
    title: "V. 高级主题和专业开发最佳实践 / V.14 微前端架构",
    slug: "42-v-v-14",
    order: 42,
  },
  {
    title: "V. 高级主题和专业开发最佳实践 / V.15部署与 DevOps for Frontend",
    slug: "43-v-v-15-devops-for-Frontend",
    order: 43,
  },
  {
    title: "V. 高级主题和专业开发最佳实践 / V.16 法律、合规与隐私",
    slug: "44-v-v-16",
    order: 44,
  },
  {
    title:
      "VI. 新兴技术和专业领域 / VI.1 渐进式 Web 应用程序 (PWA)：原生般的 Web 体验",
    slug: "45-vi-vi-1-web-pwa-web",
    order: 45,
  },
  {
    title:
      "VI. 新兴技术和专业领域 / VI.2 WebAssembly (WASM)：释放 Web 平台的原生性能",
    slug: "46-vi-vi-2-webassembly-wasm-web",
    order: 46,
  },
  {
    title:
      "VI. 新兴技术和专业领域 / VI.3 跨平台开发：移动端 (React Native, Capacitor)，桌面端 (Electron, Tauri)",
    slug: "47-vi-vi-3-react-native-capacitor-electron-tauri",
    order: 47,
  },
  {
    title: "VI. 新兴技术和专业领域 / VI.4 Web 图形，数据可视化和沉浸式体验",
    slug: "48-vi-vi-4-web",
    order: 48,
  },
  {
    title: "VI. 新兴技术和专业领域 / VI.5 Web 音频和媒体流：实时音频/视频处理",
    slug: "49-vi-vi-5-web",
    order: 49,
  },
  {
    title: "VI. 新兴技术和专业领域 / VI.6 区块链和 Web3 集成",
    slug: "50-vi-vi-6-web3",
    order: 50,
  },
  {
    title: "VI. 新兴技术和专业领域 / VI.7 前端 AI/ML 集成：浏览器内机器学习",
    slug: "51-vi-vi-7-ai-ml",
    order: 51,
  },
  {
    title:
      "VI. 新兴技术和专业领域 / VI.8 边缘计算：Cloudflare Workers、Vercel Edge Functions、Deno Deploy",
    slug: "52-vi-vi-8-cloudflare-workersvercel-edge-functionsdeno-deploy",
    order: 52,
  },
  {
    title:
      "VI. 新兴技术和专业领域 / VI.9 设备集成：Web Bluetooth、Web USB、WebHID、Generic Sensor API",
    slug: "53-vi-vi-9-web-bluetoothweb-usbwebhidgeneric-sensor-api",
    order: 53,
  },
  {
    title: "VII. 持续学习和职业发展",
    slug: "54-vii",
    order: 54,
  },
  {
    title: "VIII. 结论：前端精通之路",
    slug: "55-viii",
    order: 55,
  },
  {
    title: "附录 A：前端学习资料汇总",
    slug: "56-a",
    order: 56,
  },
];
